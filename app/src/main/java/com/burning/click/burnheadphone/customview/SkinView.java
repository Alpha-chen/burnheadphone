package com.burning.click.burnheadphone.customview;
...
//        bitmapCanvas.drawCircle(width / 2, height / 2, 150, mPaintCircle);
        bitmapCanvas.drawCircle(width / 2, height / 2, 200, mPaintCircle);
        //通过Path绘制贝塞尔曲线
        mPath.reset();
        mPath.moveTo(width, (height / 2 + 200) - (currentProgress * 400f / maxProgress));//通过此处根据进度设置高度
        mPath.lineTo(width, height / 2 + 250);
        mPath.lineTo(0, height / 2 + 200);
        mPath.lineTo(0, (height / 2 + 200) - (currentProgress * 400f / maxProgress));//通过此处根据进
